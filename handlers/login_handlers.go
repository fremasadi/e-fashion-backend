...
func LoginHandler(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = db.QueryRow("SELECT id, name, email, password FROM users WHERE email = ? AND password = ?", credentials.Email, credentials.Password).Scan(&user.ID, &user.Name, &user.Email, &user.Password)
		if err != nil {
